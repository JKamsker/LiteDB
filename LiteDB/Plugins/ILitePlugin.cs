...
namespace LiteDB.Plugins
{
    ...
    {
        void RegisterBinaryOperator(string name, BsonBinaryOperator operation, BsonExpressionType expressionType, int precedence = int.MaxValue, string source = null);

        void RegisterFunction(string name, BsonExpressionFunction function, BsonExpressionType expressionType, bool convertScalarLeftToEnumerable = true, bool isScalarResult = false);
    }
    ...
    public delegate BsonValue BsonExpressionFunction(BsonDocument root, Collation collation, BsonDocument parameters, BsonValue left, BsonValue right);
}
