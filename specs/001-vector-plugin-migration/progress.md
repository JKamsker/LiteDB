...
- Added integration coverage for lifecycle and concurrency (T035-T036): `VectorIndexLifecycle_Tests` exercises create/search/drop/recreate semantics while inspecting underlying graph state, and `VectorIndexConcurrency_Tests` stress concurrent updates vs. vector queries to validate snapshot isolation.
