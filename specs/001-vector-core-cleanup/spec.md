...
| BSON & serialization | LiteDB/Document/BsonType.cs<br>LiteDB/Document/BsonValue.cs<br>LiteDB/Document/Json/JsonWriter.cs<br>LiteDB/Utils/Extensions/BufferSliceExtensions.cs<br>LiteDB/Plugins/Bson/CustomBsonTypeDescriptor.cs<br>LiteDB/Plugins/Bson/CustomBsonTypeRegistry.cs | Uses plugin-managed BSON type registration for vector encodings (core no longer defines `BsonType.Vector`; LiteDB.Vector reserves type codes in the `0x90` range). | Implemented | Vector BSON handlers now live in `LiteDB.Vector`, registered through the core BSON type registry. |
| Storage & engine metadata | LiteDB/Engine/Pages/BasePage.cs<br>LiteDB/Engine/Pages/PageFactoryRegistry.cs<br>LiteDB/Plugins/Storage/IPageTypeRegistry.cs<br>LiteDB/Plugins/Storage/PageTypeRegistry.cs<br>LiteDB/Plugins/Indexing/IPluginIndexMetadataRegistry.cs | Provides plugin page factories and per-index metadata hooks so vector pages/metadata can be owned by `LiteDB.Vector`. | Implemented | Vector page implementations and metadata serializers now live in `LiteDB.Vector`, registered through the core page factory + index metadata registries. |
...
Early plugin hooks left four blocking gaps that prevented vector code from leaving the core. The current plugin surface resolves these gaps; the list is kept for historical context and to explain why the infrastructure exists. Each entry is tracked under `specs/001-vector-core-cleanup/gaps/`.

| Gap | Description | Impact | Blocked Components | Proposed Solution | Backward Compatibility |
|-----|-------------|--------|--------------------|-------------------|------------------------|
| `gap-query-state` | Resolved: plugin-owned query metadata bags replace vector-specific state previously stored on `Query`. | Critical | `inventory/query-planning.json` | Use `QueryMetadataBag`/`IQueryMetadataAccessor` + `QueryPlanningContext.GetOrCreateMetadata(...)` to carry planner state. | Core remains plugin-agnostic; missing-plugin behavior is host-controlled (`PluginMissingBehavior`). |
| `gap-bson-serialization` | Resolved: plugin-managed BSON type registration is available and `LiteDB.Vector` owns the vector BSON type codes via the registry (core no longer hardcodes `BsonType.Vector`). | Critical | `inventory/bson-serialization.json` | Introduce reserved type code registration that lets plugins supply serializers/deserializers without touching core enums. | Provide shims that delegate to the plugin registry while older databases remain readable during rollout. |
| `gap-storage-pipeline` | Resolved: plugin page factories and persisted index metadata allow plugins to own custom pages and on-disk payloads. | Critical | `inventory/storage-engine.json` | Register page factories via `IPageTypeRegistry` and metadata codecs via `IPluginIndexMetadataRegistry`. | Preserve existing page/type codes and metadata layout when migrating GA databases. |
| `gap-indexing-extensibility` | Resolved: plugin-owned index strategies + ensure-index interception replace core vector ensure APIs. | High | `inventory/public-api.json` | Use `IIndexStrategy`/`IIndexRegistry` for index types, `IEnsureIndexInterceptorRegistry` for interception, and `ICustomIndexStrategyRegistry` for higher-level orchestration. | Consumers use `LiteDB.Vector` extension APIs; core enforces missing-plugin policy for plugin-owned indexes. |
