...
- T008: Drafted `LiteDB/Plugins/Storage/IPageTypeRegistry.cs` and wired the default context with a registry for page factory descriptors.
...
- T040: Expanded `docs/plugins/plugin-development.md` with dedicated sections for the query metadata accessor, BSON type registry, and page factory registry, including registration samples and cross-references to `LiteDatabaseServices` so plugin authors can wire the new vector extension points.
