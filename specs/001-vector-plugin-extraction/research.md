...
## Decision 2: Default Handling for Legacy Vector Databases
- **Decision**: LiteDB will open databases even when LiteDB.Vector is absent but will refuse operations that touch vector-dependent collections/documents. The engine will mark vector indexes as "unavailable" and surface diagnostics instructing users to install the plugin; non-vector data remains accessible.
