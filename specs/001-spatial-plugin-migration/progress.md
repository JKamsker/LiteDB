...
- **2025-11-03 update**:
  ...
  - Updated `SpatialPlugin` expression-function registration to use plugin-aware delegates (root/collation/parameter-aware) and marked results as scalar so `SPATIAL_NEAR/IN_BOX/WITHIN/INTERSECTS/CONTAINS` parse and execute correctly once the plugin initializes; validated end-to-end via `LiteDB.Spatial.Core.Tests/Plugin/SpatialPluginIntegrationTests.cs` using `dotnet test LiteDB.Spatial.Core.Tests -c Release -f net8.0 --filter SpatialPluginIntegrationTests`.
