...
- [x] Mark `IPluginDiagnosticPolicy.MissingBehavior` and `PluginMissingBehavior.RefuseOperations` as `[Obsolete]` per plan.
...
- [x] Refuse or re-scope `Rebuild()` in factory mode (exclusive access requirement).
